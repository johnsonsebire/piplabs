...
function LegEditor({ side, leg, onChange }: LegEditorProps) {
 ...
 <div className={leg.enabled ? "" : " "}>
 <div className="d-flex flex-column gap-3">
 <div className="d-flex align-items-center justify-content-between">
 <Label className=" text-uppercase font-mono text-secondary">Entry Conditions</Label>
 <div className="d-flex align-items-center gap-2">
 <span className=" font-mono text-secondary text-uppercase">ALL</span>
 <Switch
 checked={leg.logic === "OR"}
 onCheckedChange={c => onChange({ ...leg, logic: c ? "OR" : "AND" })}
 
 />
 <span className=" font-mono text-secondary text-uppercase">ANY</span>
 </div>
 </div>

 <div className="d-flex flex-column gap-3">
 {leg.conditions.length === 0 && (
 <p className=" font-mono text-secondary fst-italic text-center py-2">
 ...
 <div className="row g-2 gap-2 align-items-center">
  ...
  <SelectTrigger className=" border-secondary font-mono text-uppercase">
  <SelectValue />
  </SelectTrigger>
  <SelectContent className="rounded-none border-secondary">
  {OP_OPTIONS.map(op => (
  <SelectItem key={op} value={op} className="font-mono text-uppercase">{op}</SelectItem>
  ))}
  </SelectContent>
  </Select>
  <IndicatorOrValuePicker
 ...
 className="w-100 border-dashed border-secondary text-secondary text-uppercase font-mono hover:border-success"
 ...
 <SelectTrigger className="rounded-none font-mono small"><SelectValue /></SelectTrigger>
...
export default function StrategiesPage() {
 ...
 <div className="d-flex flex-column gap-2 pt-2">
  ...
  <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "buy" | "sell")} className="w-100 pt-2">
  <TabsList className="d-grid w-100 p-0 border border-secondary" style={{ gridTemplateColumns: '1fr 1fr' }}>
  <TabsTrigger
  value="buy"
