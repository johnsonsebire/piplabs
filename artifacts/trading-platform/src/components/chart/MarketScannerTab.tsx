...
import { Play, StopCircle, Settings2, Bell, Globe, Search, Star, Activity, Zap, TrendingUp, TrendingDown } from "lucide-react";
...
export function MarketScannerTab() {
  // ── Global scanner state (persists across pages via Zustand) ──────────────
  const {
    isScanning,
    setIsScanning,
    isConnected,
  ...
  // Sync user preferences into local UI state
  useEffect(() => {
    if (user) {
  ...
  const rawDisplayItems = debouncedSearchQuery
    ? (searchResults as any[]).map((r: any) => ({ symbol: r.symbol, name: r.displayName, instrumentType: r.instrumentType, shortName: r.shortName, pip: r.pip }))
    : (dbAssets as any[]).map((a: any) => ({ symbol: a.symbol, name: a.displayName, instrumentType: a.type, shortName: a.shortName, pip: a.pipSize }));
  ...
  const toggleScanner = () => {
    ...
    if ((watchlist as any[]).length === 0) { swalError("Empty Watchlist", "Please add some assets to your favorites/watchlist."); return; }
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#070c0f", fontFamily: "Space Mono, monospace" }}>
      ...
      <div style={{ flexShrink: 0, padding: "12px", background: "linear-gradient(180deg, #0d1520 0%, #0a1018 100%)", borderBottom: "1px solid #1a2a3a" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
          ...
          {/* WS live feed connection dot */}
          <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: isConnected ? "#3b82f6" : "#334155" }} title={isConnected ? "Live feed connected" : "Feed disconnected"} />
          <div style={{ flex: 1 }} />
          <span style={{ fontSize: "9px", color: "#334155", letterSpacing: "0.05em" }}>
            {(watchlist as any[]).length} FAVORITED
          </span>
        </div>
        ...
        <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
          ...
          <div
            ...
            title="Toggle auto-start on login"
          >
      ...
      {showSettings && (
        <div style={{ flexShrink: 0, padding: "12px", background: "rgba(13,21,32,0.9)", borderBottom: "1px solid #1a2a3a" }}>
          ...
          <div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: "9px", color: "#475569", marginBottom: "4px" }}>Cooldown (mins)</div>
      ...
      <div style={{ flex: 1, overflowY: "auto", minHeight: 0 }}>
        {debouncedSearchQuery && isSearching ? (
          ...
          filteredItems.map((item) => {
            const isWatched = (watchlist as any[]).some((w: any) => w.symbol === item.symbol);
            ...
            const isScannedNow = isScanning && (watchlist as any[]).some((w: any) => w.symbol === item.symbol);
      ...
      <div style={{ flexShrink: 0, height: "130px", overflowY: "auto", background: "#03070a", borderTop: "1px solid #0d1520" }}>
        <div style={{ padding: "4px 8px", borderBottom: "1px solid #0d1520", display: "flex", alignItems: "center", gap: "6px", position: "sticky", top: 0, background: "#03070a", zIndex: 1 }}>
          ...
          <div style={{ flex: 1 }} />
          {logs.length > 0 && (
            <button onClick={clearLogs} style={{ background: "none", border: "none", color: "#1e2a3a", fontSize: "8px", cursor: "pointer", fontFamily: "Space Mono, monospace" }}>CLEAR</button>
          )}
        </div>
